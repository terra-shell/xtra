...
impl<A> Ptr<A, TxStrong> {
  pub fn new(inner: WasmRc<Chan<A>>) -> Self {
  ...
  Rc: RefCounter,
{
  ...
  pub fn inner_ptr(&self) -> *const () {
    WasmRc::as_ptr(&self.inner) as *const ()
  }
...
impl TxStrong {
  ...
  fn try_new<A>(inner: &Chan<A>) -> Option<TxStrong> {
    ...
    loop {
      ...
      match inner.sender_count.compare_exchange_weak(n, n + 1, Acquire, Relaxed) {
...
/// {}
/// ```
pub trait RefCounter: WasmSend + Sync + 'static + Unpin + private::RefCounter {}
...
mod tests {
  ...
  fn starts_with_rc_count_one() {
    let inner = WasmRc::new(Chan::new(None));
